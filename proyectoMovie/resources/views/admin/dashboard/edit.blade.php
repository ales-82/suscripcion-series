...
<section class="container w-75">
	...
	<h2 class="h2 text-center bg-secondary text-white rounded mt-2">EDITAR {{ $blog[0]->titulo }}</h2>
	<form action="{{route('abm-noticias.update', $blog[0])}}" method="POST" enctype="multipart/form-data">
		@csrf		
		<div class="form-group my-3">
			<label for="" class="fw-folder">TITULO</label>
			<input type="text" class="form-control" name="titulo" value="{{ old('titulo',$blog[0]->titulo) }}">
			@error('titulo')
		...
		<div class="form-group my-3">
			...
			<textarea name="resumen" cols="30" rows="2" class="form-control">{{ old('resumen',$blog[0]->resumen) }}</textarea>
			@error('resumen')
			<p class="text-white bg-danger mt-1">{{ $message }}</p>
			@enderror
		</div>
		<div class="form-group my-3">
			...
			<textarea name="contenido" cols="30" rows="3" class="form-control">{{ old('contenido',$blog[0]->contenido) }}</textarea>
